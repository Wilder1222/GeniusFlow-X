...
export default function DeckDetailPage() {
    ...
    const handleExportMarkdown = () => {
        ...
        toast.success(t('exportSuccess', { count: cards.length, filename }));
    };
    ...
    const handleCardSelect = (cardId: string) => {
        ...
        if (newSelected.has(cardId)) {
            ...
            if (newSelected.size >= 100) {
                // Keep this hardcoded or add to messages if needed
    ...
    if (loading) return <MainLayout>{t('loading')}</MainLayout>;
    if (!deck) return <MainLayout>{t('deckNotFound')}</MainLayout>;
    ...
    return (
        <MainLayout>
                    ...
                    <div className={styles.titleRow}>
                        ...
                        <div className={styles.actionButtons}>
                            ...
                            <button className={styles.studyButton} onClick={() => router.push(`/study?deck=${deck.id}`)}>
                                {t('startStudy', { count: cards.length })}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className={styles.content}>
                    <div className={styles.addCardSection}>
                        <div className={styles.sectionHeader}>
                            ...
                            <div className={styles.headerButtons}>
                                ...
                                >
                                    {showAddForm ? t('collapse') : t('expand')}
                                </button>
                            </div>
                        </div>
                    ...
                    <div className={styles.cardList}>
                        ...
                        <div className={styles.cardListHeader}>
                            <h2>
                                {t('existingCards', { count: filteredCards.length })}
                                ...
                                <div className={styles.batchActions}>
                                    ...
                                    >
                                        {selectedCards.size > 0 && paginatedCards.every(c => selectedCards.has(c.id))
                                            ? t('deselectAllCurrentPage')
                                            : t('selectAllCurrentPage')}
                                    </button>
                                    {selectedCards.size > 0 && (
                                        <>
                                            <span className={styles.selectionCount}>
                                                {t('selectionCount', { count: selectedCards.size })}
                                            </span>
                                            <button
                                                className={styles.deleteSelectedBtn}
                                                onClick={handleDeleteSelected}
                                            >
                                                🗑️ {t('deleteSelected', { count: selectedCards.size })}
                                            </button>
                                        </>
                                    )}
                                </div>
                        ...
                        {filteredCards.length === 0 ? (
                            <div className={styles.emptyState}>
                                {searchQuery ? t('noMatchedCards') : t('noCardsYet')}
                            </div>
                        ) : (
                                ...
                                <table className={styles.table}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ width: '35%' }}>{t('frontHeader')}</th>
                                            <th style={{ width: '35%' }}>{t('backHeader')}</th>
                                            <th style={{ width: '15%' }}>{t('statusHeader')}</th>
                                            <th style={{ width: '15%' }}>{t('actionsHeader')}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                        ...
                        {filteredCards.length > pageSize && (
                            <div className={styles.paginationContainer}>
                                <div className={styles.paginationInfo}>
                                    {t('paginationInfo', {
                                        start: startIndex + 1,
                                        end: Math.min(endIndex, filteredCards.length),
            ...
            <ConfirmDialog
                ...
                message={t('batchDeleteMessage', { count: selectedCards.size })}
                details={t('batchDeleteDetails')}
                variant="danger"
